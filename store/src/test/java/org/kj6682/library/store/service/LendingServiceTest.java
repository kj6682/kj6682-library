...
import org.kj6682.library.store.service.LendingServiceImpl;
...
public class LendingServiceTest {
	...
	public void setUp() {
		...
		oldLending = new Lending(Long.MAX_VALUE, Long.MAX_VALUE, Long.MAX_VALUE, today_minus30, today,
				org.kj6682.library.store.bean.Lending.Status.ACTIVE);
		...
		extendedLending = new Lending(Long.MAX_VALUE, Long.MAX_VALUE, Long.MAX_VALUE, today_minus30, today_plus30,
				org.kj6682.library.store.bean.Lending.Status.EXTENDED);
		deletedLending = new Lending(Long.MAX_VALUE, Long.MAX_VALUE, Long.MAX_VALUE, today_minus30, today,
				org.kj6682.library.store.bean.Lending.Status.DONE);
	...
	public void grant() {
		...
		doReturn(newLending).when(lendingDao).update(anyLong(), anyLong(), anyLong(), eq(today), eq(today_plus30),
				eq(org.kj6682.library.store.bean.Lending.Status.ACTIVE));
		...
		try {
			...
			Assert.assertEquals(org.kj6682.library.store.bean.Lending.Status.ACTIVE, result.getStatus());
	...
	public void extend() {
		...
		doReturn(extendedLending).when(lendingDao).update(anyLong(), anyLong(), anyLong(), eq(today_minus30), eq(today_plus30),
				eq(org.kj6682.library.store.bean.Lending.Status.EXTENDED));
		...
		try {
			...
			Assert.assertNotNull(result);
			Assert.assertEquals(org.kj6682.library.store.bean.Lending.Status.EXTENDED, result.getStatus());
	...
	public void delete() {
		...
		doReturn(deletedLending).when(lendingDao).update(anyLong(), anyLong(), anyLong(), eq(today_minus30), any(Date.class),
				eq(org.kj6682.library.store.bean.Lending.Status.DONE));
		...
		try {
			...
			Assert.assertNotNull(result);
			Assert.assertEquals(org.kj6682.library.store.bean.Lending.Status.DONE, result.getStatus());
			Assert.assertEquals(today_minus30, result.getFrom());
